...
function CreateTalentRow(parent,tier)
	...
	row:SetHeight(TALENT_BUTTON_SIZE + 8);
	row:SetPoint("TOPLEFT",16,-tier * (TALENT_BUTTON_SIZE + 8));
...
--	row:SetBackdropColor(0,0,0.8);
	...
	for i = 1, NUM_TALENT_COLUMNS do
		...
		btn:SetPoint("LEFT",i * (TALENT_BUTTON_SIZE + 8) + 32,0);
...
function mod:InitTalents()
	local _, class, classID = UnitClass(ex.unit);
	-- Background
...
--	ex:SetBackgroundTexture("Interface\\TalentFrame\\"..background.."-");
	...
	for i = 1, MAX_TALENT_TIERS do
		...
		local group = mod.page["row"..i];
		group.levelText:SetText(talentLevel[i]);
		group:Show();
	end
	-- Set Talents
    for tier = 1, MAX_TALENT_TIERS do
        for column=1, NUM_TALENT_COLUMNS do
            local talentID, name, iconTexture, selected, available = GetTalentInfo(tier, column, talentGroup, true, ex.unit);           
